...
# Session state initialization (Khởi tạo trạng thái hệ thống)
if "page" not in st.session_state:
    st.session_state.page = "dashboard"
if "theme" not in st.session_state:
    st.session_state.theme = "Light"  # Mặc định mở lên là giao diện Sáng sủa
...
# Load custom CSS dựa trên chế độ Dark/Light Mode được chọn
def load_css():
    # 1. Định nghĩa bộ biến màu động dựa vào st.session_state.theme
    if st.session_state.theme == "Dark":
        theme_colors = """
        --bg: #0A0A0B;      /* Nền tối sâu */
        --surface: #111113; /* Khối bề mặt tối */
        --surface2: #1A1A1E;
        --border: #2A2A30;
        --text: #E8E8EC;    /* Chữ trắng rõ */
        --text-muted: #6B6B78;
        """
    else:
        theme_colors = """
        --bg: #F7F7FA;       /* Nền sáng dịu */
        --surface: #FFFFFF;  /* Khối bề mặt trắng tinh nổi bật */
        --surface2: #EDF2F7; /* Bề mặt phụ xám nhạt */
        --border: #E2E8F0;   /* Viền xám phân cách rõ ràng */
        --text: #1A202C;     /* Chữ đen đậm tương phản cao, cực dễ đọc */
        --text-muted: #718096;
        """

    # 2. Nhúng mã CSS tổng thể (Giữ nguyên cấu trúc giao diện của bạn)
    st.markdown(f"""
    <style>
    ...
    :root {{
        /* Giữ nguyên các tông màu cảnh báo chủ đạo của SOC */
        --red: #E53E3E;
        --red-light: #FEB2B2;
        --red-dark: #9B2335;
        --orange: #DD6B20;
        ...
        /* Áp dụng bộ mã màu động đã cấu hình ở trên */
        {theme_colors}
...
# Luôn gọi hàm CSS để hiển thị đúng giao diện ngay từ đầu
load_css()
...
with st.sidebar:
    ...
    # ─── Nút gạt chuyển đổi giao diện Dark/Light Mode ───────────
    st.markdown("<div style='margin-top: 20px;'></div>", unsafe_allow_html=True)
    
    # Tạo nút gạt Toggle, dựa vào trạng thái hiện tại trong session_state
    is_dark = st.toggle("🌙 Chế độ tối (Dark Mode)", value=(st.session_state.theme == "Dark"))
    
    # Thay đổi trạng thái theme và làm mới trang để áp dụng CSS màu mới ngay lập tức
