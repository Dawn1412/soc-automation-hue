...
def render(config: dict):
    ...
    with col1:
        ...
        jobs = [
            {
                "name": "SCAN_EMAIL",
                "desc": f"Quét email từ SOC Canh Bao",
...
# Job 1: Quét email mỗi 30 phút
@scheduler.scheduled_job(IntervalTrigger(minutes=30))
def scan_job():
    print("Scanning emails...")
    # Gọi logic từ utils/email_utils.py

# Job 2: Gửi báo cáo lúc 11h45
@scheduler.scheduled_job(CronTrigger(hour=11, minute=45))
def send_job():
    print("Sending report...")
    # Gọi logic từ utils/sheets_utils.py + email_utils.py

scheduler.start()""", language="python")
